...
import {
  DURATIONS, DAY_ORDER, DAY_LABELS, DEFAULT_WEEK_SCHEDULE,
  scheduleAddBreak, scheduleRemoveBreak, scheduleUpdateBreak,
...
export default function OnboardingPage() {
  ...
  async function doSubmit(redirectTo: string) {
    ...
    // No bloqueamos el redirect si falla — el middleware verifica practitioners directamente.
    await supabase
      .from('user_profiles')
      .update({ role: 'practitioner' } as never)
      .eq('user_id', user.id)
      .then(({ error }) => { if (error) console.warn('user_profiles role update failed:', error.message) })
  ...
  const updateBreak = (key: DayKey, idx: number, f: 'start' | 'end', v: string) => setSchedule(prev => scheduleUpdateBreak(prev, key, idx, f, v))
