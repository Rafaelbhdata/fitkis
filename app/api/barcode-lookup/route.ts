...
function estimateSMAEEquivalents(
	nutrients: NutrientData,
	servingGrams: number = 100,
): { group_type: FoodGroup; quantity: number; note?: string }[] {
	const equivalents: {
		group_type: FoodGroup;
		quantity: number;
		note?: string;
	...
	if (netCarbs >= 15) {
		...
		if (protein >= 7 && netCarbs >= 20) {
			const legumEquiv = Math.min(
				floorHalf(netCarbs / 20),
				MAX_EQUIV_PER_GROUP,
			); // Legumes have ~20g carbs per equiv
			if (legumEquiv > 0) {
				equivalents.push({
	...
	try {
		...
		if (!barcode) {
			return NextResponse.json(
				{ error: 'Se requiere código de barras' },
				{ status: 400 },
			);
		}
		...
		if (cached) {
			...
			if (cachedProduct?.nutrients_per_100g) {
				...
				const servingMatch = (cachedProduct.serving_size || '').match(
					/(\d+(?:\.\d+)?)\s*(?:g|ml)/i,
				);
				const servingGrams = servingMatch ? parseFloat(servingMatch[1]) : 100;
				cachedProduct.estimated_equivalents = estimateSMAEEquivalents(
					nutriments,
					servingGrams,
				);
			}
		...
		const offResponse = await fetch(
			...
			{
				headers: {
					'User-Agent': 'FitKis/1.0 (https://fitkis.com; contact@fitkis.com)',
				},
			},
		);
